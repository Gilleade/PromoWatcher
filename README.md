...
## Como rodar o watcher

```bash
python watch_promos.py
```
...
## Como rodar a interface local

Com o watcher rodando (ou não — a interface lê o mesmo banco SQLite em
modo WAL, então pode ser aberta em paralelo sem conflito):
...
Edite `alerts.json` na raiz do projeto. Veja o formato completo e
exemplos em [docs/ALERTAS.md](docs/ALERTAS.md). O watcher recarrega o
